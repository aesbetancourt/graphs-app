...
$graph->node( {id=>0, label=>'node 0'} );   #create or update existing node.  id must be a simple scalar.
$graph->node( {id=>1, label=>'node 1'} );   #label is optional and should be string
$graph->node( {id=>2, label=>'node 2'} );
$graph->node( {id=>3, label=>'node 3'} );
$graph->edge( {sourceID=>0, targetID=>1, weight=>2} );  #create or update edge between sourceID and targetID;  weight (cost) must be > 0
